...
def inicio_jogo():
    ...
    print("\nSuas cartas são: "+A+" e "+B)
    print("\nA soma de suas cartas é "+str(E+F)+".")
    print("\nAs cartas da mesa são: "+C+" e "+D)
    ...
    for i in range(5,10):
        ...
        if resposta == "NÃO":
            for j in range(5,10):
                if mesa <17:
                    ...
                    if mamamias[j] == "A":
                        ...
                        mamamias[j] = T
                    mesa = mesa + T
                    print("\nAs cartas da mesa são "+str(EFGH[7])+"  "+str(EFGH[8])+"  "+ str(mamamias[5])+"  "+ str(mamamias[6])+" "+ str(mamamias[7])+" "+ str(mamamias[8]))
                    if mesa > 21:
                        if EFGH[2] == 11:
                            mesa -= 10
                        if EFGH[3] == 11:
            ...
            if mamamia[i] == "A":
                ...
                mamamia[i] = T
            pessoa = pessoa + T
            print("\nSuas cartas são: "+str(EFGH[5])+"  "+str(EFGH[6])+"  "+ str(mamamia[5])+"  "+ str(mamamia[6])+"  "+ str(mamamia[7])+"  "+ str(mamamia[8]))
            if pessoa > 21:
                if EFGH[0] == 11:
                    pessoa -= 10
                if EFGH[1] == 11:
...
while 1 == 1:
    ...
    if (meio_do_jogo(EFGH)) == "Mesa":
        fichas -= aposta
        print("\nA Mesa ganhou! Você tem "+str(fichas)+ " fichas.")
    else:
        fichas += aposta
        print("\nVocê ganhou! Você tem "+str(fichas)+ " fichas.")
    if fichas == 0:
